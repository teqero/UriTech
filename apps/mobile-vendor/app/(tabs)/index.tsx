...
import { countByStatus, INITIAL_VENDOR_ORDERS } from '../../lib/orders';

const VENDOR_RED = '#EE2737';

export default function VendorDashboardScreen() {
  const novos = countByStatus(INITIAL_VENDOR_ORDERS, 'novos');
  const preparando = countByStatus(INITIAL_VENDOR_ORDERS, 'preparando');
  const prontos = countByStatus(INITIAL_VENDOR_ORDERS, 'prontos');
  const entregues = countByStatus(INITIAL_VENDOR_ORDERS, 'historico');

  const goOrders = (tab: string) => {
    router.push({ pathname: '/(tabs)/orders', params: { tab } } as never);
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.menuRow}>
        <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/(tabs)/menu' as never)}>
          <Text style={styles.menuEmoji}>📋</Text>
          <Text style={styles.menuText}>Cardápio</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/(tabs)/orders' as never)}>
          <Text style={styles.menuEmoji}>🏷️</Text>
          <Text style={styles.menuText}>Promoções</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/(tabs)/analytics' as never)}>
          <Text style={styles.menuEmoji}>📊</Text>
